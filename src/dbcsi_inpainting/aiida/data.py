...
__all__ = ("InpaintingStructure", "BatchedStructures", "BatchedStructuresData",
           "InpaintingResults", "InpaintingResultsData", "convert_structure")
...
def convert_structure(structure: ase.Atoms | Structure, strct_type: str = 'ase') -> ase.Atoms | Structure:
    """Convert a structure to the specified type."""
    if strct_type not in ['ase', 'pymatgen']:
        raise ValueError(f"Unknown structure type: {strct_type}. Available types are 'ase' and 'pymatgen'.")
...
class BatchedStructures:
    ...
    def get_structure(self, key: str, strct_type: str = 'ase') -> ase.Atoms | Structure:
        """Return a single structure by key."""
        if key not in self.keys:
            raise ValueError(f"Key '{key}' not found in the available structures.")

        return convert_structure(self.structures[key], strct_type=strct_type)
        

    def get_structures(self, strct_type: str = 'ase') -> dict[str, ase.Atoms | Structure]:
        """Return all structures as a dictionary."""
        return {key: convert_structure(self.structures[key], strct_type=strct_type) for key in self.keys}
...
class BatchedStructuresData(Data):
    ...
    def structures_to_file(self, structures, name) -> None:
        ...
        with tempfile.NamedTemporaryFile(mode='w+', suffix='extxyz') as handle:
            ...
            self.base.repository.put_object_from_filelike(handle, f'{name}.extxyz')
    ...
    def get_structure(self, key: str, strct_type: str = 'ase') -> ase.Atoms | Structure:
        ...
        with self.base.repository.open(self.file_name, mode='rb') as handle:
            structure = ase.io.read(io.TextIOWrapper(handle), index=self.keys.index(key), format='extxyz')
    ...
    def get_structures(self, strct_type: str = 'ase') -> dict[str, ase.Atoms | Structure]:
        ...
        with self.base.repository.open(self.file_name, mode='rb') as handle:
            ase_structures = ase.io.read(io.TextIOWrapper(handle), index=':', format='extxyz')
            
        if strct_type == 'ase':
            return {str(ase_atom.info['key']): ase_atom for ase_atom in ase_structures}
        elif strct_type == 'pymatgen':
            return {str(ase_atom.info['key']): AseAtomsAdaptor.get_structure(ase_atom) for ase_atom in ase_structures}
        else:
            raise ValueError(f"Unknown structure type: {strct_type}. Available types are 'ase' and 'pymatgen'.")
...
class InpaintingResults:
    ...
    def __init__(self, structures: dict, relaxed_structures: dict = None):
        ...
        if relaxed_structures:
            if set(structures.keys()) != set(relaxed_structures.keys()):
                raise ValueError("Keys of structures and relaxed_structures must match.")
        ...
        self._structures = structures
        self._relaxed_structures = relaxed_structures if relaxed_structures else {}
    ...
    def get_structure(self, key: str, relaxed: bool = False, strct_type: str = 'ase') -> ase.Atoms | Structure:
        """Return a single structure by key."""
        if key not in self.keys:
            raise ValueError(f"Key '{key}' not found in the available structures.")
        if relaxed:
            ...
            return convert_structure(self.relaxed_structures[key], strct_type=strct_type)
        else:
            return convert_structure(self.structures[key], strct_type=strct_type)
        

    def get_structures(self, relaxed: bool = False, strct_type: str = 'ase') -> dict[str, ase.Atoms | Structure]:
        """Return all structures as a dictionary."""
        if relaxed:
            if not self.relaxed_structures:
                raise ValueError("No relaxed structures available.")
            return {key: convert_structure(self.relaxed_structures[key], strct_type=strct_type) for key in self.keys}
        else:
            return {key: convert_structure(self.structures[key], strct_type=strct_type) for key in self.keys}

class InpaintingResultsData(Data):
    
    def __init__(self, structures: dict, relaxed_structures: dict = None, **kwargs):
        ...
        if relaxed_structures is not None:
            ase_relaxed_structures = self.structures_to_atoms(relaxed_structures)
            self.structures_to_file(ase_relaxed_structures, 'relaxed_structures')
    ...
    def from_inpainting_results(cls, inpainting_results: InpaintingResults):
        ...
        return cls(structures=inpainting_results.structures, relaxed_structures=inpainting_results.relaxed_structures)
    ...
    def get_structure(self, key: str, relaxed: bool = False, strct_type: str = 'ase') -> ase.Atoms | Structure:
        """Return a single structure by key."""
        file_name = 'relaxed_structures.extxyz' if relaxed else 'structures.extxyz'
        if file_name not in self.base.repository.list_object_names():
            raise ValueError(f"No {'relaxed ' if relaxed else ''}structures found in the repository.")
        if key not in self.keys:
            raise ValueError(f"Key '{key}' not found in the available structures.")
