...
from dbcsi_inpainting.sampling import (  # run_all_experiments can be defined in sampling.py or a separate module
    run_all_experiments,
)
from dbcsi_inpainting.utils.data_utils import (
    create_dataloader,
    load_structures,
    prepare_dataset,
)
from dbcsi_inpainting.utils.misc import get_mattergen_unknown_formulas
...
def main(
    ...
    structures_subset: list[Structure] = load_structures(
        formulas_to_choose, N_structures, max_num_atoms
    )
    dataset = prepare_dataset(structures_subset)
    N_samples_per_structure = param_grid["N_samples_per_structure"]
    batch_size = param_grid["batch_size"]
    structures_dl = create_dataloader(
        dataset, N_samples_per_structure, batch_size
    )
    ...
    run_all_experiments(
        ...
        param_grid,
        fix_cell,
        relaxation_kwargs=relax_kwargs,
    )
